...
struct aui_widget {
	...
	unsigned int mapped;
	struct widget_primitives primitives;
...
struct aui_container {
	...
	struct aui_rowpar *rowconfigure;
	struct aui_columnpar *columnconfigure;
...
struct aui_window {
	...
	const char *title;
	int draw_flag;
	struct aui_window_config config;
...
struct aui_frame {
	struct aui_container con;
	struct aui_frame_config config;
};

struct aui_canvas_item {
	unsigned int type;
	struct widget_primitives primitives;
	struct aui_canvas_item_config config;
	TAILQ_ENTRY(aui_canvas_item) entries;
};

struct aui_canvas {
	struct aui_container con;
	TAILQ_HEAD(aui_canvas_item_queue, aui_canvas_item) queue;
};
...
void widget_init(struct aui_widget*, unsigned int, struct widget_ops*);
